...
export function AdminLoginPage() {
  ...
  return <main className="login-page"><div className="login-language"><LanguagePicker /></div><form className="login-card" onSubmit={submit}><span className="brand-symbol large">E</span><p className="eyebrow">{t('brand')}</p><h1>{t('admin')}</h1>{error && <p className="alert">{error}</p>}<label>{t('email')}<input name="email" type="email" required autoComplete="email" /></label><label>{t('password')}<input name="password" type="password" required autoComplete="current-password" /></label><button className="button primary wide" disabled={busy}>{busy ? t('loading') : t('login')}</button></form></main>;
}
